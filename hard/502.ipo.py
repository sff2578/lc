...
class Solution:
    def findMaximizedCapital(
        ...
        # use max heap, add all posible profits to heap, pop the top one, this one contributs to the new capital
        # no max heap in python, use negitive of value
        # Input: k = 2, w = 0, profits = [1,2,3], capital = [0,1,1]
        # Input: k = 3, w = 0, profits = [1,2,3], capital = [0,1,2]
        heap = []
        checked = 0
        prev_w = -1
        while k > 0:
            if checked < len(capital):
                for i in range(len(capital)):
                    if capital[i] <= w and capital[i] > prev_w:
